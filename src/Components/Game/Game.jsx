...
export default function Game() {
  ...
  let { seconds, minutes, start, restart, isRunning } = useTimer({
    expiryTimestamp,
    autoStart: false,
  });
  ...
  const handleClick = (pickedSquare) => {
    if (isRunning) {
      if (pickedSquare === square) {
        ...
        setRecord((prevState) => [
          ...prevState,
          { square: square, correct: true },
        ]);
      } else {
        setRecord((prevState) => [
          ...prevState,
          { square: square, correct: false },
        ]);
      }
      generateSquare();
    }
    ...
    if (record.correct) {
      return (
        <div className="score-board--record-data">
          <img
            className="score-board--record_correct_logo"
            alt="Check mark"
            src={checkmark}
          />
          <span className="score-board--record_correct_square">
            {record.square}
          </span>
        </div>
      );
      ...
      return (
        <div className="score-board--record-data">
          <img
            className="score-board--record_wrong_logo"
            alt="X Mark"
            src={xmark}
          />
          <span className="score-board--record_wrong_square">
            {record.square}
          </span>
        </div>
      );
    ...
    <div className="App">
      <main className="App-header">
        ...
        <div className="App-game">
          ...
          <section className="score-board w-full bg-white  py-5 flex items-start sm:items-center justify-between px-5 sm:px-10 shadow rounded-t">
            <span className="score-board--title text-grey-200 text-lg font-bold">
              Score : {score}
            </span>

            <span className="score-board--timer text-white text-lg font-bold">
              {minutes}:{seconds}
            </span>
            <div className="score-board--record">{recordElements}</div>
            <button
              onClick={() => {
                generateSquare();
                setScore(0);
